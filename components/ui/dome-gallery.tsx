...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const origRelative = { left: originalPos.left - rootRect.left, top: originalPos.top - rootRect.top, width: originalPos.width, height: originalPos.height };
      requestAnimationFrame(() => {
        animatingOverlay.style.left = origRelative.left + 'px'; animatingOverlay.style.top = origRelative.top + 'px';
        animatingOverlay.style.width = origRelative.width + 'px'; animatingOverlay.style.height = origRelative.height + 'px';
      ...
      animatingOverlay.addEventListener('transitionend', () => {
        ...
        requestAnimationFrame(() => {
          ...
          requestAnimationFrame(() => { el.style.opacity = '1'; setTimeout(() => { el.style.transition = ''; el.style.opacity = ''; openingRef.current = false; unlockScroll(); }, 300); });
        });
      }, { once: true });
    };
  ...
  return (
    <>
      ...
      <div ref={rootRef} className="sphere-root relative w-full h-full"
        style={{ ['--segments-x' as any]: segments, ['--segments-y' as any]: segments, ['--overlay-blur-color' as any]: overlayBlurColor } as React.CSSProperties}>
        <main ref={mainRef} className="absolute inset-0 grid place-items-center overflow-hidden select-none bg-transparent" style={{ touchAction: 'none', WebkitUserSelect: 'none' }}>
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div key={`${it.x},${it.y},${i}`} className="sphere-item absolute m-auto"
                  ...
                  <div className="item__image absolute block overflow-hidden cursor-pointer bg-gray-200" role="button" tabIndex={0} aria-label={it.alt || 'Open image'}
                    onClick={e => {
          ...
          <div className="absolute inset-0 m-auto z-[3] pointer-events-none"
            style={{ WebkitMaskImage: `radial-gradient(rgba(235,235,235,0) 70%, var(--overlay-blur-color,${overlayBlurColor}) 90%)`, maskImage: `radial-gradient(rgba(235,235,235,0) 70%, var(--overlay-blur-color,${overlayBlurColor}) 90%)`, backdropFilter: 'blur(3px)' }} />
