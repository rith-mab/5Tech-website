...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }: OrbitItemProps) {
  ...
  return (
    ...
    >
      <div style={{ transform: `rotate(${-rotation}deg)` }} className="w-full h-full flex items-center justify-center">{item}</div>
    </motion.div>
  );
}
...
export default function OrbitImages({
  ...
  const items = images.map((src, index) => (
    <div
      key={src}
      className="w-full h-full p-4 flex items-center justify-center bg-white/70 dark:bg-card/70 border border-slate-100 dark:border-slate-800 rounded-[24px] shadow-sm hover:shadow-md transition-all duration-300 backdrop-blur-md group"
    >
      <img
        src={src}
